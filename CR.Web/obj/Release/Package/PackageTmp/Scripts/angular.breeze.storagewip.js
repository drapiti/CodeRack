...
(function () {
    ...
    storageModule.provider('zStorageConfig', function () {
        this.config = {
            // These are the properties we need to set lun
            enabled: false,
            key: '',
            events: {
                error: 'store.error',
     ...
     * 
     * For internal use by lun and zStorageWip services
     */
            ...
            try {
                ...
                if (importMeta.breezeVersion === storeMeta.breezeVersion &&
                    ...
                    _broadcast(storeConfig.events.error,
                        'Did not load from lun because mismatched versions',
                        { current: storeMeta, storage: importMeta });
                }
            } catch (ex) {
                _broadcast(storeConfig.events.error, 'Exception during load from lun: ' + ex.message, ex);
            }
            return null; // failed
        }
     ...
     * breeze work in progress (WIP) from local lun
     */
